...
CREATE OR REPLACE PROCEDURE public.drop_debug_tables(_infoOnly boolean DEFAULT true, INOUT _message text DEFAULT '')
    LANGUAGE plpgsql
    AS $$
/****************************************************
**
...
CALL cap.update_task_context(_debugMode => true);
CALL sw.update_context(_debugMode => true);

SELECT * FROM predefined_analysis_datasets(1500, _infoOnly => false, _previewSql => false, _populateTempTable => true);
SELECT * FROM t_tmp_predefinedanalysisdatasets;

CALL drop_debug_tables(_infoOnly => true);
CALL drop_debug_tables(_infoOnly => false);
