--
-- Name: update_cached_requested_run_batch_stats(integer, text, text); Type: PROCEDURE; Schema: public; Owner: d3l243
--

CREATE OR REPLACE PROCEDURE public.update_cached_requested_run_batch_stats(IN _batchid integer DEFAULT 0, INOUT _message text DEFAULT ''::text, INOUT _returncode text DEFAULT ''::text)
    LANGUAGE plpgsql
    AS $$
/****************************************************
**
                ...
                SELECT RRB.batch_id,
                       ...
                       CASE
                           ...
                           ELSE ROUND(EXTRACT(epoch FROM
                                        (CURRENT_TIMESTAMP - (COALESCE(ActiveStatsQ.oldest_active_request_created, StatsQ.oldest_request_created)))) / (86400)::numeric)
                       END AS days_in_queue
                FROM ( SELECT batch_id
                       FROM T_Requested_Run_Batches
                       WHERE T_Requested_Run_Batches.batch_id > 0
                     ) RRB
                     LEFT OUTER JOIN
                     ( SELECT RR.batch_id AS batch_id,
                       ...
                       WHERE RR.batch_id > 0 AND
                             (RR.batch_id = _batchID OR _batchID = 0)
                     ...
                     ( SELECT RR.batch_id AS batch_id,
                       ...
                       FROM t_requested_run RR
                       WHERE RR.batch_id > 0 AND
                             RR.state_name = 'Active'::citext AND
                             (RR.batch_id = _batchID OR _batchID = 0)
                       GROUP BY RR.batch_id
                     ) ActiveStatsQ ON RRB.batch_id = ActiveStatsQ.batch_id
                WHERE RRB.batch_id = _batchID OR _batchID = 0
              ) AS s
                     ...
                     ( SELECT RR.batch_id,
                           ...
                           SUM(CASE
                               WHEN ((LOWER(COALESCE(spr.block_and_randomize_runs, '')) = 'yes') AND
                                    ((COALESCE(RR.block, 0) = 0) OR (COALESCE(RR.run_order, 0) = 0))) THEN 1
                                    ELSE 0
...
ALTER PROCEDURE public.update_cached_requested_run_batch_stats(IN _batchid integer, INOUT _message text, INOUT _returncode text) OWNER TO d3l243;

--
-- Name: PROCEDURE update_cached_requested_run_batch_stats(IN _batchid integer, INOUT _message text, INOUT _returncode text); Type: COMMENT; Schema: public; Owner: d3l243
--

COMMENT ON PROCEDURE public.update_cached_requested_run_batch_stats(IN _batchid integer, INOUT _message text, INOUT _returncode text) IS 'UpdateCachedRequestedRunBatchStats';
