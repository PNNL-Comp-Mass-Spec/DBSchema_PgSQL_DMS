...
**    _changeErrorsToErrorIgnore   When true, if _messageType is 'Error' will update messages in public.t_log_entries to have type 'ErrorIgnore' (if duplicates are removed)
**
**  Usage:
**
**      SELECT * FROM public.consolidate_log_messages (_infoOnly => true);
**      SELECT * FROM public.consolidate_log_messages (_changeErrorsToErrorIgnore => false, _infoOnly => true);
**      SELECT * FROM public.consolidate_log_messages ('Error', '%132%', _keepFirstMessageOnly => false, _infoOnly => true);
**      SELECT * FROM public.consolidate_log_messages ('Error', '%132%', _infoOnly => false);
**
**  Auth:   mem
...
BEGIN
    ...
    If Not _infoOnly Then
        ...
        WHERE EXISTS (SELECT D.entry_id
                      ...
                      WHERE D.entry_id = public.t_log_entries.entry_id);
        --
        ...
        _message := format ('Found %s duplicate %s in public.t_log_entries; deleted %s %s',
...
ALTER FUNCTION public.consolidate_log_messages(_messagetype text, _messagefilter text, _keepfirstmessageonly boolean, _changeerrorstoerrorignore boolean, _infoonly boolean) OWNER TO d3l243;

--
-- Name: FUNCTION consolidate_log_messages(_messagetype text, _messagefilter text, _keepfirstmessageonly boolean, _changeerrorstoerrorignore boolean, _infoonly boolean); Type: COMMENT; Schema: public; Owner: d3l243
--

COMMENT ON FUNCTION public.consolidate_log_messages(_messagetype text, _messagefilter text, _keepfirstmessageonly boolean, _changeerrorstoerrorignore boolean, _infoonly boolean) IS 'ConsolidateLogMessages';
