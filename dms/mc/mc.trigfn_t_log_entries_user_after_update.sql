...
BEGIN
    ...
    SET entered_by = CASE WHEN strpos(Coalesce(entered_by, ''), ' (') > 0 THEN
                               format('%s%s%s)', Left(mc.t_log_entries.entered_by, strpos(Coalesce(entered_by, ''), ' (') - 1), _sepChar, _userInfo)
                          WHEN mc.t_log_entries.entered_by IS NULL THEN SESSION_USER
                          ELSE format('%s%s%s)', Coalesce(mc.t_log_entries.entered_by, '??'), _sepChar, _userInfo)
                     END
