 ...
 SELECT js.processor,
   ...
   FROM (sw.t_job_steps js
     JOIN ( SELECT t_job_steps.processor,
          ...
          GROUP BY t_job_steps.processor, t_job_steps.step_tool) dateq ON (((js.processor OPERATOR(public.=) dateq.processor) AND (js.step_tool OPERATOR(public.=) dateq.step_tool))))
  WHERE (COALESCE(js.processor, ''::public.citext) OPERATOR(public.<>) ''::public.citext)
  GROUP BY js.processor, js.step_tool, (EXTRACT(year FROM js.start)), (EXTRACT(month FROM js.start)), dateq.start_max;
