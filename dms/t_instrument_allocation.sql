...
CREATE TRIGGER trig_t_instrument_allocation_after_update AFTER UPDATE ON public.t_instrument_allocation FOR EACH ROW WHEN (((old.allocated_hours IS DISTINCT FROM new.allocated_hours) OR ((old.comment)::text IS DISTINCT FROM (new.comment)::text))) EXECUTE FUNCTION public.trigfn_t_instrument_allocation_after_update();
