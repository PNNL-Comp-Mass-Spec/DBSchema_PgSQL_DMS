...
--
-- Name: add_update_analysis_job_request(text, text, text, text, text, text, text, text, text, text, text, text, text, text, integer, text, integer, text, text, text); Type: PROCEDURE; Schema: public; Owner: d3l243
--
-- Overload 2

CREATE OR REPLACE PROCEDURE public.add_update_analysis_job_request(IN _datasets text, IN _requestname text, IN _toolname text, IN _paramfilename text, IN _settingsfilename text, IN _protcollnamelist text, IN _protcolloptionslist text, IN _organismname text, IN _organismdbname text, IN _requesterusername text, IN _comment text, IN _specialprocessing text, IN _datapackageid text, IN _state text, INOUT _requestid integer DEFAULT 0, IN _mode text DEFAULT 'add'::text, IN _autoremovenotreleaseddatasets integer DEFAULT 0, INOUT _message text DEFAULT ''::text, INOUT _returncode text DEFAULT ''::text, IN _callinguser text DEFAULT ''::text)
    LANGUAGE plpgsql
    AS $$
/****************************************************
**
...
**      Add new analysis job request to request queue
**
**      This is an overloaded version of add_update_analysis_job_request where _dataPackageID is text
**      This procedure is used by the DMS website, e.g. https://dmsdev2.pnl.gov/analysis_job_request/create/analysis_job/2297536
**
**  Arguments:
**    _datasets                         Comma-separated list of datasets
**    _requestName                      Job request name
...
BEGIN
    ...
    BEGIN
        ...
        --
        -- _message := format('Calling add_update_analysis_job_request with _requestID %s and _dataPackageID %s', _requestID, _dataPackageID);
        -- CALL post_log_entry('Debug', _message, 'add_update_analysis_job_request');
        -- _message := '';
        ...
        CALL add_update_analysis_job_request (
                ...
                _state                            => _state,
                _requestID                        => _requestId,            -- Input/Output
                _mode                             => _mode,
