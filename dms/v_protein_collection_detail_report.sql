...
CREATE VIEW public.v_protein_collection_detail_report AS
 SELECT lookupq.protein_collection_id AS id,
    ...
    string_agg((lookupq.organism_storage_path)::text, ', '::text ORDER BY (lookupq.organism_storage_path)::text) AS organism_storage_path,
    lookupq.state,
    lookupq.entries,
    lookupq.residues,
        CASE
   ...
   FROM (( SELECT DISTINCT pc.protein_collection_id,
            ...
            pc.description,
            orglist.organism,
            orglist.storage_location AS organism_storage_path,
