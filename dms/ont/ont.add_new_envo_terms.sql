--
-- Name: add_new_envo_terms(public.citext, integer, integer); Type: FUNCTION; Schema: ont; Owner: d3l243
--

CREATE OR REPLACE FUNCTION ont.add_new_envo_terms(_sourcetable public.citext DEFAULT 'T_Tmp_ENVO'::public.citext, _infoonly integer DEFAULT 1, _previewdeleteextras integer DEFAULT 1) RETURNS TABLE(item_type public.citext, entry_id integer, term_pk public.citext, term_name public.citext, identifier public.citext, is_leaf public.citext, synonyms public.citext, parent_term_id public.citext, parent_term_name public.citext, grandparent_term_id public.citext, grandparent_term_name public.citext, entered timestamp without time zone, updated timestamp without time zone)
    LANGUAGE plpgsql
    AS $$
/****************************************************
**
...
BEGIN
    ...
    ---------------------------------------------------
    -- Set matches_existing to 1 for rows that match an existing row in ont.t_cv_envo
    ---------------------------------------------------
    --
    UPDATE Tmp_SourceData t
    ...
    -- <a1>
        ...
        RAISE INFO 'Updated % rows in ont.t_cv_envo using %', Cast(_myRowCount as varchar(9)), _sourceTable;

        ---------------------------------------------------
        -- Add new rows
        ---------------------------------------------------
        --
        INSERT INTO ont.t_cv_envo (term_pk, term_name, identifier, is_leaf, synonyms,
        ...
        RAISE INFO 'Added % new rows to ont.t_cv_envo using %', Cast(_myRowCount as varchar(9)), _sourceTable;
...
ALTER FUNCTION ont.add_new_envo_terms(_sourcetable public.citext, _infoonly integer, _previewdeleteextras integer) OWNER TO d3l243;

--
-- Name: FUNCTION add_new_envo_terms(_sourcetable public.citext, _infoonly integer, _previewdeleteextras integer); Type: COMMENT; Schema: ont; Owner: d3l243
--

COMMENT ON FUNCTION ont.add_new_envo_terms(_sourcetable public.citext, _infoonly integer, _previewdeleteextras integer) IS 'AddNewENVOTerms';
