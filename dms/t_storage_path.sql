...
CREATE TRIGGER trig_t_storage_path_after_update AFTER UPDATE ON public.t_storage_path FOR EACH ROW WHEN (((old.storage_path OPERATOR(public.<>) new.storage_path) OR ((old.machine_name)::text IS DISTINCT FROM (new.machine_name)::text) OR ((old.vol_name_client)::text IS DISTINCT FROM (new.vol_name_client)::text))) EXECUTE FUNCTION public.trigfn_t_storage_path_after_update();
...
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE public.t_storage_path TO writeaccess;
