...
BEGIN
    ...
    BEGIN
        ...
        If Exists (Select * FROM Tmp_RequestedRuns WHERE Request_ID Is Null) Then
            ...
            _logErrors := false;
            _message := 'Requested runs must be integers, not names; first invalid item: ' || Coalesce(_firstInvalid, '');
        ...
        If _count > 0 Then
            ...
            _logErrors := false;
            _message := 'Requested run list contains requests that do not exist: ' || _invalidIDs;
