...
CREATE VIEW public.v_req_run_instrument_picklist_ex AS
 SELECT t_instrument_name.instrument AS val,
  ...
  WHERE ((NOT (t_instrument_name.instrument OPERATOR(public.~~) 'SW_%'::public.citext)) AND (t_instrument_name.status = 'active'::bpchar) AND (t_instrument_name.operations_role OPERATOR(public.<>) 'QC'::public.citext))
UNION
