...
CREATE VIEW public.v_analysis_job_and_dataset_archive_state AS
 SELECT j.job,
        CASE
            WHEN ((j.job_state_id = 1) AND ((da.archive_state_id = ANY (ARRAY[3, 4, 10, 14, 15])) OR (da.archive_state_last_affected < (CURRENT_TIMESTAMP - '03:00:00'::interval)))) THEN (asn.job_state)::text
            WHEN ((j.job_state_id = 1) AND (da.archive_state_id < 3)) THEN ((asn.job_state)::text || ' (Dataset Not Archived)'::text)
            WHEN ((j.job_state_id = 1) AND (da.archive_state_id > 3)) THEN ((((asn.job_state)::text || ' (Dataset '::text) || (dasn.archive_state)::text) || ')'::text)
   ...
   FROM ((public.t_dataset_archive_state_name dasn
     JOIN public.t_dataset_archive da ON ((dasn.archive_state_id = da.archive_state_id)))
     RIGHT JOIN ((public.t_analysis_job j
     JOIN public.t_analysis_job_state asn ON ((j.job_state_id = asn.job_state_id)))
     JOIN public.t_dataset d ON ((j.dataset_id = d.dataset_id))) ON ((da.dataset_id = d.dataset_id)));
