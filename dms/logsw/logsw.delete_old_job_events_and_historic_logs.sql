--
-- Name: delete_old_job_events_and_historic_logs(boolean, integer, text, text); Type: PROCEDURE; Schema: logsw; Owner: d3l243
--

CREATE OR REPLACE PROCEDURE logsw.delete_old_job_events_and_historic_logs(IN _infoonly boolean DEFAULT false, IN _yearfilter integer DEFAULT 0, INOUT _message text DEFAULT ''::text, INOUT _returncode text DEFAULT ''::text)
    LANGUAGE plpgsql
    AS $$
/****************************************************
**
**  Desc:
**      Delete entries over 2 years old in logsw.t_job_events, logsw.t_job_step_events,
**      logsw.t_job_step_processing_log, and logsw.t_log_entries
**
...
BEGIN
    ...
    BEGIN
        ...
        Else
            ...
            Else
                DELETE FROM logsw.t_job_events Target
                WHERE EXISTS (SELECT 1
                              FROM Tmp_JobEventIDs Src
            ...
            _jobEventsMessage := format('%s %s old entries from logsw.t_job_events %s; event ID range %s to %s',
                                        _jobEventsMessage, _eventsToDelete, _thresholdDescription, _eventIdMin, _eventIdMax);

            If Not _infoOnly Then
                CALL post_log_entry ('Normal', _jobEventsMessage, 'delete_old_job_events_and_historic_logs', _targetSchema => 'logsw');
            End If;
        End If;
        ...
        Else
            ...
            Else
                DELETE FROM logsw.t_job_step_events Target
                WHERE EXISTS (SELECT 1
                              FROM Tmp_JobStepEventIDs Src
                              WHERE target.event_id = Src.Event_ID);
            ...
            End If;
             ...
             _jobStepEventsMessage := format('%s %s old entries from logsw.t_job_step_events %s; event ID range %s to %s',
                                             _jobStepEventsMessage, _eventsToDelete, _thresholdDescription, _eventIdMin, _eventIdMax);

            If Not _infoOnly Then
                CALL post_log_entry ('Normal', _jobStepEventsMessage, 'delete_old_job_events_and_historic_logs', _targetSchema => 'logsw');
            End If;
        End If;
        ...
        Else
            ...
            Else
                DELETE FROM logsw.t_job_step_processing_log Target
                WHERE EXISTS (SELECT 1
            ...
            End If;
             ...
             _jobStepProcessingLogMessage := format('%s %s old entries from logsw.t_job_step_processing_log %s; event ID range %s to %s',
                                                    _jobStepProcessingLogMessage, _eventsToDelete, _thresholdDescription, _eventIdMin, _eventIdMax);

            If Not _infoOnly Then
                CALL post_log_entry ('Normal', _jobStepProcessingLogMessage, 'delete_old_job_events_and_historic_logs', _targetSchema => 'logsw');
            End If;
        End If;
        ...
        Else
            ...
            Else
                DELETE FROM logsw.t_log_entries Target
            ...
            _historicLogMessage := format('%s %s old entries from logsw.t_log_entries %s; entry ID range %s to %s',
                                          _historicLogMessage, _logEntriesToDelete, _thresholdDescription, _eventIdMin, _eventIdMax);

            If Not _infoOnly Then
                CALL post_log_entry ('Normal', _historicLogMessage, 'delete_old_job_events_and_historic_logs', _targetSchema => 'logsw');
            End If;
        End If;
        ...
        DROP TABLE Tmp_JobEventIDs;
        DROP TABLE Tmp_JobStepEventIDs;
    ...
    EXCEPTION
        ...
        If Not _infoOnly Then
            CALL post_log_entry ('Error', _exceptionMessage, 'delete_old_job_events_and_historic_logs', _targetSchema => 'logsw');
        End If;
    ...
    DROP TABLE IF EXISTS Tmp_JobEventIDs;
    DROP TABLE IF EXISTS Tmp_JobStepEventIDs;
...
ALTER PROCEDURE logsw.delete_old_job_events_and_historic_logs(IN _infoonly boolean, IN _yearfilter integer, INOUT _message text, INOUT _returncode text) OWNER TO d3l243;
