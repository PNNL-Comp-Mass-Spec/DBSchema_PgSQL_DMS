...
CREATE VIEW public.v_assigned_storage_report AS
 SELECT t_instrument_name.instrument,
    ((storage.vol_name_client)::text || (storage.storage_path)::text) AS "?column?",
   ...
   FROM ((public.t_instrument_name
     JOIN ( SELECT t_storage_path.storage_path_id,
          ...
          WHERE ((t_storage_path.storage_path_function)::text = ('inbox'::bpchar)::text)) src ON ((t_instrument_name.source_path_id = src.storage_path_id)))
     JOIN ( SELECT t_storage_path.storage_path_id,
            t_storage_path.storage_path,
            t_storage_path.vol_name_client,
            t_storage_path.vol_name_server
           FROM public.t_storage_path
          WHERE ((t_storage_path.storage_path_function)::text = ('raw-storage'::bpchar)::text)) storage ON ((t_instrument_name.storage_path_id = storage.storage_path_id)));
