...
CREATE VIEW public.v_get_pipeline_job_processors AS
 SELECT j.job,
  ...
  WHERE ((pg.group_enabled OPERATOR(public.=) 'Y'::public.citext) AND (pgm.membership_enabled OPERATOR(public.=) 'Y'::public.citext) AND ((j.job_state_id = ANY (ARRAY[1, 2, 8, 20])) OR ((j.job_state_id = 4) AND (j.finish > (CURRENT_TIMESTAMP - '02:00:00'::interval))) OR ((j.job_state_id = 5) AND (j.start > (CURRENT_TIMESTAMP - '30 days'::interval)))))
...
COMMENT ON VIEW public.v_get_pipeline_job_processors IS 'Returns jobs with state new, in progress, holding, or pending, jobs completed within the last 2 hours, and jobs failed within the last 30 days';
