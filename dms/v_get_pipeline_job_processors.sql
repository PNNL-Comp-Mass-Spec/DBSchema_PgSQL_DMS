...
CREATE VIEW public.v_get_pipeline_job_processors AS
 SELECT j.job,
  ...
  WHERE ((pg.group_enabled = 'Y'::bpchar) AND (pgm.membership_enabled = 'Y'::bpchar) AND ((j.job_state_id = ANY (ARRAY[1, 2, 8])) OR ((j.job_state_id = 4) AND (j.finish > (CURRENT_TIMESTAMP - '02:00:00'::interval))) OR ((j.job_state_id = 5) AND (j.start > (CURRENT_TIMESTAMP - '30 days'::interval)))))
