...
**  Usage:
**
**      SELECT * FROM cap.consolidate_log_messages (_infoOnly => true);
**      SELECT * FROM cap.consolidate_log_messages (_changeErrorsToErrorIgnore => false, _infoOnly => true);
**      SELECT * FROM cap.consolidate_log_messages ('Error', '%132%', _keepFirstMessageOnly => false, _infoOnly => true);
**      SELECT * FROM cap.consolidate_log_messages ('Error', '%132%', _infoOnly => false);
**
**  Auth:   mem
...
BEGIN
    ...
    If Not _infoOnly Then
        ...
        WHERE EXISTS (SELECT D.entry_id
                      ...
                      WHERE D.entry_id = cap.t_log_entries.entry_id);
        --
        ...
        _message := format ('Found %s duplicate %s in cap.t_log_entries; deleted %s %s',
        ...
        If _duplicateMessageCount > 0 Or _deletedMessageCount > 0 Then
            CALL public.post_log_entry ('Normal', _message, 'Consolidate_Log_Messages', 'cap');
        End If;

        If _deletedMessageCount > 0 And _changeErrorsToErrorIgnore Then
            UPDATE cap.t_log_entries target
...
ALTER FUNCTION cap.consolidate_log_messages(_messagetype text, _messagefilter text, _keepfirstmessageonly boolean, _changeerrorstoerrorignore boolean, _infoonly boolean) OWNER TO d3l243;

--
-- Name: FUNCTION consolidate_log_messages(_messagetype text, _messagefilter text, _keepfirstmessageonly boolean, _changeerrorstoerrorignore boolean, _infoonly boolean); Type: COMMENT; Schema: cap; Owner: d3l243
--

COMMENT ON FUNCTION cap.consolidate_log_messages(_messagetype text, _messagefilter text, _keepfirstmessageonly boolean, _changeerrorstoerrorignore boolean, _infoonly boolean) IS 'ConsolidateLogMessages';
