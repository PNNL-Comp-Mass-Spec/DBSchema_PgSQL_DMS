...
CREATE TRIGGER trig_t_organisms_after_update AFTER UPDATE ON public.t_organisms FOR EACH ROW WHEN (((old.organism OPERATOR(public.<>) new.organism) OR ((old.short_name)::text IS DISTINCT FROM (new.short_name)::text) OR ((old.domain)::text IS DISTINCT FROM (new.domain)::text) OR ((old.kingdom)::text IS DISTINCT FROM (new.kingdom)::text) OR ((old.phylum)::text IS DISTINCT FROM (new.phylum)::text) OR ((old.class)::text IS DISTINCT FROM (new.class)::text) OR ((old."order")::text IS DISTINCT FROM (new."order")::text) OR ((old.family)::text IS DISTINCT FROM (new.family)::text) OR ((old.genus)::text IS DISTINCT FROM (new.genus)::text) OR ((old.species)::text IS DISTINCT FROM (new.species)::text) OR ((old.strain)::text IS DISTINCT FROM (new.strain)::text) OR ((old.newt_id_list)::text IS DISTINCT FROM (new.newt_id_list)::text) OR (old.ncbi_taxonomy_id IS DISTINCT FROM new.ncbi_taxonomy_id) OR (old.active IS DISTINCT FROM new.active))) EXECUTE FUNCTION public.trigfn_t_organisms_after_update();
...
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE public.t_organisms TO writeaccess;
GRANT INSERT,UPDATE ON TABLE public.t_organisms TO pceditor;
