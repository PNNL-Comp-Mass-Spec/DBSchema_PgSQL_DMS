--
-- Name: trigfn_t_protein_collections_after_update_all(); Type: FUNCTION; Schema: pc; Owner: d3l243
--

CREATE OR REPLACE FUNCTION pc.trigfn_t_protein_collections_after_update_all() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
/****************************************************
**
**  Desc:
**      Prevents updating all rows in t_protein_collections,
...
BEGIN
    ...
    If _updatedRowCount > 1 And _updatedRowCount >= _existingRowCount Then
        _message := format('Cannot update all %s rows in %s; use a WHERE clause to limit the affected rows (see trigger function %s)',
                           _updatedRowCount, 't_protein_collections', 'trigfn_t_protein_collections_after_update_all');
...
ALTER FUNCTION pc.trigfn_t_protein_collections_after_update_all() OWNER TO d3l243;
