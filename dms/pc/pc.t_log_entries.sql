...
CREATE TRIGGER trig_t_log_entries_after_update AFTER UPDATE ON pc.t_log_entries FOR EACH ROW WHEN (((old.entered <> new.entered) OR (old.posted_by IS DISTINCT FROM new.posted_by) OR (old.type IS DISTINCT FROM new.type) OR (old.message IS DISTINCT FROM new.message))) EXECUTE FUNCTION pc.trigfn_t_log_entries_user_after_update();
