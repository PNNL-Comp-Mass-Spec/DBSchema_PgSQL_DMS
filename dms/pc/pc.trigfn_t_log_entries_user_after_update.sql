...
BEGIN
    ...
    _userInfo := format('%s; %s', timestamp_text(localtimestamp), SESSION_USER);

    _sepChar := ' (';

    UPDATE pc.t_log_entries
    SET entered_by = CASE WHEN strpos(Coalesce(entered_by, ''), ' (') > 0 THEN
                               format('%s%s%s)', Left(pc.t_log_entries.entered_by, strpos(Coalesce(entered_by, ''), ' (') - 1), _sepChar, _userInfo)
                          WHEN pc.t_log_entries.entered_by IS NULL THEN SESSION_USER
                          ELSE format('%s%s%s)', Coalesce(pc.t_log_entries.entered_by, '??'), _sepChar, _userInfo)
                     END
