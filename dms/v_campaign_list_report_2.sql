...
CREATE VIEW public.v_campaign_list_report_2 AS
 SELECT c.campaign_id AS id,
    c.campaign,
    c.state,
    public.get_campaign_role_person(c.campaign_id, 'Technical Lead'::text) AS technical_lead,
    public.get_campaign_role_person(c.campaign_id, 'PI'::text) AS pi,
    public.get_campaign_role_person(c.campaign_id, 'Project Mgr'::text) AS project_mgr,
    c.project,
    c.description,
    c.created,
