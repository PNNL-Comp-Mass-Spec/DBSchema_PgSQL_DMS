...
CREATE VIEW public.v_service_center_use_detail_report AS
 SELECT u.report_id,
    ...
    ds.cc_report_state_id AS cost_center_report_state_id,
        CASE
            WHEN ((rep.report_state_id = ANY (ARRAY[1, 2])) AND (ds.cc_report_state_id = 3)) THEN 'Submitting to service center'::public.citext
            WHEN ((rep.report_state_id = ANY (ARRAY[1, 2])) AND (ds.cc_report_state_id = 5)) THEN 'Refunding to service center'::public.citext
            ELSE scrs.cc_report_state
   ...
   FROM ((((((((svc.t_service_use u
     ...
     LEFT JOIN public.t_dataset_svc_center_report_state scrs ON ((ds.cc_report_state_id = scrs.cc_report_state_id)))
