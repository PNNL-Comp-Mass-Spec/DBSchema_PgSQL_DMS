...
CREATE VIEW public.v_eus_export_job_metadata AS
 SELECT d.dataset_id,
    ...
    e.experiment,
    o.organism,
    aj.job AS analysis_job,
    antool.analysis_tool,
    antool.result_type AS analysis_result_type,
    aj.protein_collection_list,
    aj.results_folder_name AS analysis_job_results_folder,
    public.combine_paths((v_dataset_folder_paths.archive_folder_path)::text, (aj.results_folder_name)::text) AS folder_path_aurora
   FROM (((((((((public.t_dataset d
     ...
     JOIN public.t_analysis_job aj ON ((d.dataset_id = aj.dataset_id)))
