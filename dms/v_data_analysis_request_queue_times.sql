   ...
   FROM ( SELECT r.id AS request_id,
           ...
           FROM ((public.t_data_analysis_request r
             LEFT JOIN ( SELECT t_data_analysis_request_updates.request_id,
                  ...
                  GROUP BY t_data_analysis_request_updates.request_id, t_data_analysis_request_updates.new_state_id) closedq ON ((closedq.request_id = r.id)))
             LEFT JOIN ( SELECT t_data_analysis_request_updates.request_id,
                    t_data_analysis_request_updates.new_state_id AS state_id,
                    min(t_data_analysis_request_updates.entered) AS statefirstentered
                   FROM public.t_data_analysis_request_updates
                  GROUP BY t_data_analysis_request_updates.request_id, t_data_analysis_request_updates.new_state_id) stateenteredq ON (((stateenteredq.request_id = r.id) AND (stateenteredq.state_id = r.state))))) outerq;
