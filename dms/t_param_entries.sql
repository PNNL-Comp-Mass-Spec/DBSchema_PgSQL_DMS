...
CREATE TRIGGER trig_t_param_entries_after_update AFTER UPDATE ON public.t_param_entries FOR EACH ROW WHEN (((old.param_file_id <> new.param_file_id) OR (old.entry_sequence_order IS DISTINCT FROM new.entry_sequence_order) OR ((old.entry_type)::text IS DISTINCT FROM (new.entry_type)::text) OR ((old.entry_specifier)::text IS DISTINCT FROM (new.entry_specifier)::text) OR ((old.entry_value)::text IS DISTINCT FROM (new.entry_value)::text))) EXECUTE FUNCTION public.trigfn_t_param_entries_after_update();
