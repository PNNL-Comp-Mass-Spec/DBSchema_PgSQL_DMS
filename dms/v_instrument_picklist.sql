...
CREATE VIEW public.v_instrument_picklist AS
 SELECT (((t_instrument_name.instrument)::text || ' '::text) || (t_instrument_name.usage)::text) AS val,
  ...
  WHERE ((NOT (t_instrument_name.instrument OPERATOR(public.~~) 'SW_%'::public.citext)) AND (t_instrument_name.status = ANY (ARRAY['active'::bpchar, 'offline'::bpchar])) AND (t_instrument_name.operations_role OPERATOR(public.<>) 'QC'::public.citext));
