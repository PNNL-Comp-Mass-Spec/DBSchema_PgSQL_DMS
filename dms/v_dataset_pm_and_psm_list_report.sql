...
CREATE VIEW public.v_dataset_pm_and_psm_list_report AS
 SELECT pm.dataset,
   ...
   FROM (((((public.v_mts_pm_results_list_report pm
     ...
     LEFT JOIN public.v_analysis_job_psm_list_report psm ON (((psm.dataset_id = pm.dataset_id) AND (psm.state_id <> ALL (ARRAY[5, 13, 14])))));
...
GRANT SELECT ON TABLE public.v_dataset_pm_and_psm_list_report TO writeaccess;
