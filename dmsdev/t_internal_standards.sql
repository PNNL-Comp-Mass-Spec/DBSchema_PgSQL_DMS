...
CREATE TABLE public.t_internal_standards (
    ...
    type public.citext,
    active public.citext DEFAULT 'A'::bpchar NOT NULL,
    CONSTRAINT ck_t_internal_standards_active CHECK (((active OPERATOR(public.=) 'A'::public.citext) OR (active OPERATOR(public.=) 'I'::public.citext))),
    CONSTRAINT ck_t_internal_standards_type CHECK (((type OPERATOR(public.=) 'All'::public.citext) OR ((type OPERATOR(public.=) 'Postdigest'::public.citext) OR (type OPERATOR(public.=) 'Predigest'::public.citext))))
);
