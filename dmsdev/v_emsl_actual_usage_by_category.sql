...
CREATE VIEW public.v_emsl_actual_usage_by_category AS
 SELECT (public.get_fiscal_year_text_from_date(run_date))::public.citext AS fy,
    proposal_id,
    category,
    (sum(duration) / (60)::numeric) AS actual_hours_used
  ...
  GROUP BY (public.get_fiscal_year_text_from_date(run_date)), proposal_id, category;
