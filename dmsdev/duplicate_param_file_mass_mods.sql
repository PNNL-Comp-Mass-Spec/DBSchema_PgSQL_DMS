--
-- Name: duplicate_param_file_mass_mods(integer, integer, boolean, boolean, text, text); Type: PROCEDURE; Schema: public; Owner: d3l243
--

CREATE OR REPLACE PROCEDURE public.duplicate_param_file_mass_mods(IN _sourceparamfileid integer, IN _destparamfileid integer, IN _updateparamentries boolean DEFAULT true, IN _infoonly boolean DEFAULT false, INOUT _message text DEFAULT ''::text, INOUT _returncode text DEFAULT ''::text)
    LANGUAGE plpgsql
    AS $$
/****************************************************
**
...
**
**      Requires that the new parameter file exists in t_param_files, but it cannot have any entries in t_param_file_mass_mods
**
**  Arguments:
...
**                          However, table t_param_entries is only used by SEQUEST parameter files, and SEQUEST was retired in 2019, so this argument is effectively obsolete
...
BEGIN
    ...
    -----------------------------------------
    -- Show the SQL for viewing mass mod info for the source or destination file
    -----------------------------------------
    ...
    _sql := 'SELECT PFMM.*, R.residue_symbol, '
     ...
     format('WHERE PFMM.param_file_id = %s ', CASE WHEN _infoOnly THEN _sourceParamFileID ELSE _destParamFileID END) ||
    ...
    -----------------------------------------
    -- Construct the format specifier used to either preview the source parameter file mass mods,
    -- or show the data after adding rows to t_param_file_mass_mods
    -----------------------------------------

    _formatSpecifier := '%-12s %-10s %-15s %-18s %-14s %-15s %-15s %-7s %-19s %-17s %-12s %-20s %-12s';
    ...
    If _infoOnly Then
        ...
        If _updateParamEntries And Exists (SELECT param_file_id FROM t_param_entries WHERE param_file_id = _sourceParamFileID) Then

            RAISE INFO '';

            _formatSpecifierPE := '%-14s %-20s %-15s %-35s %-15s %-12s';
...
ALTER PROCEDURE public.duplicate_param_file_mass_mods(IN _sourceparamfileid integer, IN _destparamfileid integer, IN _updateparamentries boolean, IN _infoonly boolean, INOUT _message text, INOUT _returncode text) OWNER TO d3l243;

--
-- Name: PROCEDURE duplicate_param_file_mass_mods(IN _sourceparamfileid integer, IN _destparamfileid integer, IN _updateparamentries boolean, IN _infoonly boolean, INOUT _message text, INOUT _returncode text); Type: COMMENT; Schema: public; Owner: d3l243
--

COMMENT ON PROCEDURE public.duplicate_param_file_mass_mods(IN _sourceparamfileid integer, IN _destparamfileid integer, IN _updateparamentries boolean, IN _infoonly boolean, INOUT _message text, INOUT _returncode text) IS 'DuplicateParamFileMassMods';
