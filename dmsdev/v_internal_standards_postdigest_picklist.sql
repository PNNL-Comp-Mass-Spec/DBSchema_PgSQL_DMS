...
CREATE VIEW public.v_internal_standards_postdigest_picklist AS
 SELECT internal_standard_id AS id,
  ...
  WHERE ((active OPERATOR(public.=) 'A'::public.citext) AND (type OPERATOR(public.=) ANY (ARRAY['Postdigest'::public.citext, 'All'::public.citext])) AND (internal_standard_id > 0));
...
GRANT SELECT ON TABLE public.v_internal_standards_postdigest_picklist TO writeaccess;
